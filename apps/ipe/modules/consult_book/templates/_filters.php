...
<div class="sf_admin_filter">
  ...
  <form action="<?php echo url_for('book_consult_book_collection', array('action' => 'filter')) ?>" method="post">
    <table cellspacing="0">
      <tbody>
                        <?php $first = true ?>
                <?php $action = true ?>
                    ...
                    <?php else: ?>
                        ...
                        <?php if (($widgets_total <= 1) && $action): ?>
                                <td rowspan="<?php echo ceil(count($configuration->getFormFilterFields($form)) / WIDGET_PER_ROW) ?>" style="border:1px solid #DDD;vertical-align:bottom;text-align:right;">
                                    ...
                                    <?php echo link_to(__('Reset', array(), 'sf_admin'), 'book_consult_book_collection', array('action' => 'filter'), array('query_string' => '_reset', 'method' => 'post')) ?> 
